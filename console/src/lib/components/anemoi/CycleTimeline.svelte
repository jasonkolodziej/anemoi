...
<div class="space-y-2">
	...
	<div class="relative h-6 w-full overflow-hidden rounded-sm bg-surface-raised">
		{#each plan.stages as stage (stage.stage)}
			<div
				class="absolute top-0 h-full border-r border-bg/60 bg-action/25"
				style={`left: ${pct(stage.start)}%; width: ${pct(stage.end_target) - pct(stage.start)}%;`}
				title={`${stageLabel[stage.stage] ?? stage.stage}: ${formatUtc(stage.start)} → ${formatUtc(stage.end_target)}`}
			></div>
			<div
				class="absolute top-0 h-full bg-[repeating-linear-gradient(45deg,color-mix(in_oklab,var(--color-action)_18%,transparent),color-mix(in_oklab,var(--color-action)_18%,transparent)_3px,transparent_3px,transparent_6px)]"
				style={`left: ${pct(stage.end_target)}%; width: ${pct(stage.end_max) - pct(stage.end_target)}%;`}
				title={`${stageLabel[stage.stage] ?? stage.stage} max budget: through ${formatUtc(stage.end_max)}`}
			></div>
		{/each}
