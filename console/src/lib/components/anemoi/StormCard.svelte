  ...
  <Card class="h-full hover:border-border-strong">
    <CardHeader>
      <CardTitle class="font-mono">
        {storm.storm_id}
        {#if storm.name}
          <span class="font-display font-normal text-text-muted"
            >{storm.name}</span
          >
        {/if}
      </CardTitle>
    ...
    <CardContent class="space-y-2">
      <div class="flex items-baseline gap-2">
        <span class="font-data text-2xl font-medium text-text"
          >{storm.latest_fix.max_wind_kt}</span
        >
        <span class="text-xs text-text-muted"
          >kt · peak {storm.peak_wind_kt}kt</span
        >
      </div>
      ...
      <div class="flex items-center justify-between pt-1">
        <Badge variant="outline"
          >{qualityLabel[storm.latest_fix.quality] ??
            storm.latest_fix.quality}</Badge
        >
        <span class="font-data text-[11px] text-text-faint"
          >{formatUtc(storm.latest_fix.valid_time)}</span
        >
      </div>
      ...
      {#if storm.last_cycle}
        <div class="text-xs text-text-faint">
          last cycle <span class="font-data text-text-muted"
            >{storm.last_cycle}</span
          >
        </div>
      {/if}
    </CardContent>
