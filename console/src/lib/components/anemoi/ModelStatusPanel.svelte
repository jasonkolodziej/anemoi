  ...
  <CardContent class="space-y-1 pt-2">
    {#if rows.length === 0}
      <p class="text-xs text-text-faint">
        No contributor weights on this cycle.
      </p>
    {/if}
      ...
      >
        <div class="flex items-baseline justify-between gap-2">
          <span class="flex items-center gap-1.5">
            ...
            <span class="font-data text-[10px] text-text-faint"
              >{row.god.direction}</span
            >
          </span>
          <span class="font-data text-xs text-text-muted"
            >{(row.weight * 100).toFixed(0)}%</span
          >
        </div>
        <div class="font-data text-[10.5px] text-text-faint">
          {row.god.role}
        </div>
        <div
          class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-surface-raised"
        >
          <div
    ...
    {#if missingRows.length > 0}
      <div class="mt-3 space-y-1 border-t border-border pt-3">
        <p
          class="font-display text-[11px] font-semibold tracking-wide text-text-faint uppercase"
        >
          Not weighing in
        </p>
        {#each missingRows as row (row.god.slug)}
          <div
            class="flex items-baseline justify-between gap-2 py-0.5 opacity-60"
          >
            <span class="flex items-center gap-1.5">
              <span
                class="h-2 w-2 shrink-0 rounded-full border border-border-strong"
              ></span>
    ...
    <p class="pt-1 text-[11px] text-text-faint">
      Fusion weights are inversely proportional to recent validation error
      (§6.1). Hover a row to isolate its track on the map.
    </p>
  </CardContent>
</Card>
