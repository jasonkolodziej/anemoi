...
| Models | **All five Group 1 models now have real Stage A/B runners (#22), `anemoi train --model {lstm,cnn,transformer,gnn,pinn}`.** LSTM (`training.real_run`) is track-only, no gridded-field cache needed. CNN/Transformer/GNN (`real_run_cnn`/`real_run_transformer`/`real_run_gnn`) read real cached GriddedFields (all 10 fields) in place of not-yet-fetched real GOES imagery/station networks -- CNN as a channel stack, Transformer the same trimmed to its fixed 40x40 grid (its patch embedding isn't resolution-agnostic like CNN's), GNN as a lattice-graph mesh (subsampled cells as nodes, 4-connectivity edges, node/edge feature widths matching `build_gnn`'s own defaults exactly), batched block-diagonal across samples. PINN (`real_run_pinn`) is architecturally different -- a residual corrector, not a predictor from raw input -- so it trains a small internal LSTM each run as the candidate track to correct, plus a real environment-feature vector from cached GriddedFields, with `models.pinn.physics_residuals` folded into the loss (only over the 12-48h leads, the uniformly-12h-spaced prefix its constant-dt_hours assumption is actually valid for). All five upload every stage's checkpoint to R2, register in `ModelRegistry`, and run `promotion.evaluate_promotion`. Verified real end-to-end on the training VM for LSTM (Stage A->B val loss 1.27->1.05, real checkpoints in R2, registered as `lstm v1`). **`training.orchestrator.run_schedule`'s injected `runner` is now wired to all five** (`training.real_orchestrator.RealOrchestratorRunner`, `anemoi train-schedule`) -- `latents`/`diffusion`/`fusion` (no real implementation yet) report a clean "not implemented" failure rather than silently no-op'ing, and `run_schedule`'s own dependency semantics correctly skip whatever depended on them while every Group 1 model still trains for real | Real runners for `diffusion`/`fusion` (Anemoi-Spread, fusion consensus); run CNN/Transformer/GNN/PINN for real on the VM once era5_cache/gdas_cache have fetched enough data |
...
| `test_real_orchestrator.py` | `RealOrchestratorRunner` dispatches each Group 1 model to its real runner; registers + evaluates promotion (including a second run comparing against the first as incumbent); non-train and unimplemented-model tasks fail cleanly instead of crashing; a real training failure is reported, not raised; full `orchestrator.build_schedule`+`run_schedule` integration trains all five Group 1 models and correctly skips latents/diffusion/fusion (no real implementation yet) in both sequential and parallel mode |
