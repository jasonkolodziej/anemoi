...
| Models | **All seven Group 1 + derived models now have real training runs (#22), plus real latent extraction wiring them together.** `anemoi train --model {lstm,cnn,transformer,gnn,pinn}` covers the five Group 1 models: LSTM (`training.real_run`) is track-only, no gridded-field cache needed. CNN/Transformer/GNN (`real_run_cnn`/`real_run_transformer`/`real_run_gnn`) read real cached GriddedFields (all 10 fields) in place of not-yet-fetched real GOES imagery/station networks -- CNN as a channel stack, Transformer the same trimmed to its fixed 40x40 grid (its patch embedding isn't resolution-agnostic like CNN's), GNN as a lattice-graph mesh (subsampled cells as nodes, 4-connectivity edges, node/edge feature widths matching `build_gnn`'s own defaults exactly), batched block-diagonal across samples. PINN (`real_run_pinn`) is architecturally different -- a residual corrector, not a predictor from raw input -- so it trains a small internal LSTM each run as the candidate track to correct, plus a real environment-feature vector from cached GriddedFields, with `models.pinn.physics_residuals` folded into the loss (only over the 12-48h leads, the uniformly-12h-spaced prefix its constant-dt_hours assumption is actually valid for). All five upload every stage's checkpoint to R2, register in `ModelRegistry`, and run `promotion.evaluate_promotion`, and now also return a `real_run.RunArtifacts` (trained model + exact standardisation stats) for real latent extraction to consume. Verified real end-to-end on the training VM for LSTM (Stage A->B val loss 1.27->1.05, real checkpoints in R2, registered as `lstm v1`). **`training.orchestrator.run_schedule`'s injected `runner` is wired to all seven tasks** (`training.real_orchestrator.RealOrchestratorRunner`, `anemoi train-schedule`): the "latents" task (`training.real_latents.extract_joint_latents`) runs each trained Group 1 model's `.encode()` over the same real HURDAT2 windows to build a real joint conditioning vector plus a real synoptic context vector and each model's own real Stage B forecast (un-standardised back to absolute lat/lon/wind using its exact `RunArtifacts` stats -- refitting those independently would only approximate the real inverse of a model's output space); diffusion (`real_run_diffusion`, the standard DDPM epsilon-prediction objective over `models.diffusion.build_diffusion`'s cosine schedule) and fusion (`real_run_fusion`, masked absolute-space MSE over `models.fusion.build_fusion`'s learned per-lead-time weights) both train against that real per-window data and register with the real `latent_signature` computed from the Group 1 versions they were built against (§5.7 model-set pinning). `run_schedule`'s own dependency semantics still correctly skip whatever a failed upstream task depended on. **Real end-to-end run on the VM (2026-09-17):** LSTM (v2) and CNN (v1) both completed for real, registered, and tagged in MLflow; Transformer then hit a real CUDA OOM training full-batch against the real (and growing) cached dataset -- `docs/streaming_dataloader.md` scopes the durable fix (mini-batched training + a streaming CPU-side loader), not yet built; `--n-augment 1` is the real working stopgap in the meantime | Build the streaming DataLoader (`docs/streaming_dataloader.md`) before running Transformer/GNN at full real cache scale; until then, run CNN/Transformer/GNN/PINN/diffusion/fusion with a reduced `--n-augment` |
...
| `test_real_run_transformer.py` | Same coverage as test_real_run_cnn.py for the Transformer baseline, plus: the real 41x41 cached crop is trimmed to the model's fixed 40x40 grid size, a cache smaller than grid_size is rejected |
| `test_real_run_gnn.py` | Lattice mesh topology (node/edge shapes, bidirectional edges, center node); node feature width matches `build_gnn`'s own defaults; block-diagonal batching never lets one sample's edges reference another's nodes; masked-loss GNN training reduces loss; full curriculum against a fake checkpoint store |
| `test_real_run_pinn.py` | Real environment-vector width/finiteness; skip-if-not-cached; displacement<->absolute coordinate round trip (zero-displacement sanity check); masked-loss PINN training (against an internally-trained candidate LSTM) reduces loss; full curriculum against a fake checkpoint store |
| `test_real_orchestrator.py` | `RealOrchestratorRunner` dispatches each Group 1/diffusion/fusion model to its real runner; registers + evaluates promotion (including a second run comparing against the first as incumbent, and diffusion registering with the real `latent_signature` computed from the Group 1 versions it was trained against); the latents task fails cleanly if Group 1 is incomplete and succeeds once it isn't; diffusion/fusion fail cleanly if latents haven't run yet; an unknown train task fails cleanly instead of crashing; a real training failure is reported, not raised; full `orchestrator.build_schedule`+`run_schedule` integration trains all seven tasks to completion in both sequential and parallel mode |
| `test_real_latents.py` | `extract_joint_latents` against five genuinely-trained (tiny, few-epoch) real models: per-model latent widths match each architecture's own `ModelSpec.latent_dim`, predictions are real finite absolute (lat, lon, wind) coordinates, context is real and finite, train/val actually both populate; rejects a `trained_artifacts` dict missing a Group 1 model |
| `test_real_run_diffusion.py` | Masked DDPM noise-prediction training reduces loss and produces real val metrics + `DiffusionArtifacts`; rejects empty train/val latent sets; full curriculum uploads a checkpoint and registers as Stage B |
| `test_real_run_fusion.py` | Masked absolute-space consensus training beats any single noisy-but-unbiased input model's own error on the set it was fit to; rejects empty train/val latent sets; a val set built with a different Group 1 model count surfaces `ConsensusFusion.forward`'s own real mismatch error; full curriculum uploads a checkpoint and registers as Stage B |
